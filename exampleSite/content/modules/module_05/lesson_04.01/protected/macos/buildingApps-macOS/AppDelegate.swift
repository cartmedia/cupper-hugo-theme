...
class AppDelegate: NSObject, NSApplicationDelegate, NSUserNotificationCenterDelegate {
  ...
  func applicationDidFinishLaunching(_ aNotification: Notification) {
    ...
    var registeredDraggedTypes: [NSPasteboard.PasteboardType] = [.URL, .fileURL, .png, .string]

    let filePromiseReceiverTypes = NSFilePromiseReceiver.readableDraggedTypes.map { NSPasteboard.PasteboardType($0) }

    registeredDraggedTypes.append(contentsOf: filePromiseReceiverTypes)

    if let button = self.statusBarItem.button {
      // Do not attemp to move to extension, waste of time
      button.registerForDraggedTypes(registeredDraggedTypes)
